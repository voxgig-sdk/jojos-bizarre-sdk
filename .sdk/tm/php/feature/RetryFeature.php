...
// JojosBizarre SDK retry feature
...
class JojosBizarreRetryFeature extends JojosBizarreBaseFeature
{
    ...
    public function init(JojosBizarreContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (JojosBizarreContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(JojosBizarreContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(JojosBizarreContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
