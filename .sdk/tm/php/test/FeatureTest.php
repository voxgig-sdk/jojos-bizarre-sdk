...
class FtCtrl extends JojosBizarreControl
{
...
class FtClient
{
    ...
    private JojosBizarreUtility $utility;

    public function __construct(JojosBizarreUtility $utility, array $options)
    {
    ...
    public function get_utility(): JojosBizarreUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (JojosBizarreContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public JojosBizarreContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (JojosBizarreContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = JojosBizarreConfig::make_feature($name);
    ...
    private static function buildUrl(JojosBizarreSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof JojosBizarreError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new JojosBizarreResponse($fetched);
        }
    ...
    private function fail(JojosBizarreContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (JojosBizarreContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
