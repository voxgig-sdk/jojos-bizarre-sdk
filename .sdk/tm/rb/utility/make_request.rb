...
module JojosBizarreUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = JojosBizarreResponse.new({})
    ...
    if fetch_err
      ...
      response = JojosBizarreResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = JojosBizarreResponse.new(fetched)
    else
