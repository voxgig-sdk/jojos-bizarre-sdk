...
//     (\`new JojosBizarreSDK(...)\` / \`JojosBizarreSDK.test(...)\` become
//     \`JojosBizarreSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'JojosBizarreSDK'
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"stand":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new JojosBizarreSDK(...)\` and any
// \`JojosBizarreSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+JojosBizarreSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/JojosBizarreSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
