...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.JOJOSBIZARRE_TEST_LIVE ||
    'TRUE' === process.env.JOJOSBIZARRE_TEST_OVERRIDE
  ) {
  ...
  m.JOJOSBIZARRE_TEST_EXPLAIN = process.env.JOJOSBIZARRE_TEST_EXPLAIN || m.JOJOSBIZARRE_TEST_EXPLAIN
