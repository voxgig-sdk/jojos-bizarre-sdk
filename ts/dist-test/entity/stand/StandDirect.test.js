...
(0, node_test_1.describe)('StandDirect', async () => {
    ...
    (0, node_test_1.test)('direct-exists', async () => {
        const sdk = new __1.JojosBizarreSDK({
    ...
    (0, node_test_1.test)('direct-load-stand', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup({ id: 'direct01' });
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-load-stand', setup.live))
            return;
    ...
    (0, node_test_1.test)('direct-list-stand', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-stand', setup.live))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
...
function liveScenariosActive() { return false && process.env.JOJOS_BIZARRE_TEST_LIVE === 'TRUE'; }
function directSetup(mockres) {
    ...
    if (live) {
        ...
        const client = new __1.JojosBizarreSDK(Object.assign({}, (0, utility_1.liveClientOptions)(), { system: { fetch: transport.fetch },
        }));
        let idmap = env['JOJOS_BIZARRE_TEST_STAND_ENTID'];
    ...
    const client = new __1.JojosBizarreSDK({
        base: 'http://localhost:8080',
...
//# sourceMappingURL=StandDirect.test.js.map