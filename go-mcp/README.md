...
[MCP](https://modelcontextprotocol.io) server exposing the JojosBizarre SDK as
two agent tools — `jojos-bizarre_list` and `jojos-bizarre_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jojos-bizarre-mcp)
make build

# 2. Provide credentials via the environment
export JOJOS_BIZARRE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user jojos-bizarre \
  -- /absolute/path/to/jojos-bizarre-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./jojos-bizarre-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// jojos-bizarre_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// jojos-bizarre_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jojos-bizarre-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user jojos-bizarre \
     -- "$PWD"/dist/*/jojos-bizarre-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `jojos-bizarre_list` and `jojos-bizarre_load` tools now appear
   in new sessions. Ask the agent to *"list character using jojos-bizarre"*
   and it calls `jojos-bizarre_list` with `{"entity":"character"}`.
...
```sh
export JOJOS_BIZARRE_APIKEY=sk_live_xxx            # API key
export JOJOS_BIZARRE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `jojos-bizarre_list` tool
...
### Call the `jojos-bizarre_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `jojos-bizarre_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `jojos-bizarre_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `JOJOS_BIZARRE_APIKEY` | API key sent with every request. |
| `JOJOS_BIZARRE_BASE` | Optional override of the API base URL. |
...
```sh
./jojos-bizarre-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"jojos-bizarre_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
