...
boru-driven command-line client **and** interactive REPL for the JojosBizarre
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/jojos-bizarre-cli)
make build

# 2. See usage (words, entities, env vars)
./jojos-bizarre-cli --help
...
./jojos-bizarre-cli load 1 character            # {id:1} shorthand
./jojos-bizarre-cli load '{id:1}' character       # explicit match map
./jojos-bizarre-cli list stand

# 5. Override the API base URL for a single call
JOJOS_BIZARRE_BASE=https://api.example.com ./jojos-bizarre-cli list character

# 6. No arguments -> interactive REPL
./jojos-bizarre-cli
jojos-bizarre> list character
jojos-bizarre> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/jojos-bizarre-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/jojos-bizarre-cli list character
   ```
...
```sh
./jojos-bizarre-cli load 1 character          # scalar shorthand for {id:1}
./jojos-bizarre-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `jojos-bizarre>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/jojos-bizarre-cli`. |
...
(`./jojos-bizarre-cli <expr>`) and interactively (the REPL), and expressions compose the
