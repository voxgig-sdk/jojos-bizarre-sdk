...
// JojosBizarre SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class JojosBizarreTestFeature extends JojosBizarreBaseFeature
{
    ...
    public function init(JojosBizarreContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (JojosBizarreContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = JojosBizarreParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (JojosBizarreContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(JojosBizarreContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = JojosBizarreParam::call($ctx, $k);
