...
class StandEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = JojosBizarreSDK::test(null, null);
        $ent = $testsdk->Stand(null);
    ...
    public function test_stream(): void
    {
        ...
        $base = JojosBizarreSDK::test($seed, null);
        $seen = iterator_to_array($base->Stand(null)->stream("list", null, null), false);
        ...
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            ...
            $got = [];
            foreach ($sdk->Stand(null)->stream("list", null, null) as $item) {
    ...
    {
        $setup = stand_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["list", "load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "stand." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $stand_ref01_data = null;
        if (count($stand_ref01_data_raw) > 0) {
...
function stand_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/stand/StandTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["stand01", "stand02", "stand03"] as $k) {
