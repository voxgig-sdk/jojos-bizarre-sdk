...
require_relative 'JojosBizarre_types'
...
class JojosBizarreSDK
  ...
  def initialize(options = {})
    ...
    utility = JojosBizarreUtility.new
    @_utility = utility
    ...
    feature_opts = JojosBizarreHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = JojosBizarreHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, JojosBizarreFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    JojosBizarreUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = JojosBizarreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = JojosBizarreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = JojosBizarreHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = JojosBizarreHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = JojosBizarreError.new(
        "graphql_error", "JojosBizarreSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = JojosBizarreSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
