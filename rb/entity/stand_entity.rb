...
class StandEntity
  ...
  def make
    opts = @_entopts.dup
    StandEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = JojosBizarreHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Stand, Hash] the current Stand data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = JojosBizarreHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Stand fields)
  def match_get
  ...
  # Load a single Stand.
  #
  # @param reqmatch [StandLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Stand.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Stand, Hash] the loaded Stand; raises JojosBizarreError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = JojosBizarreHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [StandListMatch, Hash, nil] match filter (any subset of
  #   Stand fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Stand>, Array] the matching Stand items; raises JojosBizarreError on failure
