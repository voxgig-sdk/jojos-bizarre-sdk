...
func TestCharacterEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set JOJOSBIZARRE_TEST_CHARACTER_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func characterBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("JOJOSBIZARRE_TEST_CHARACTER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["JOJOSBIZARRE_TEST_CHARACTER_ENTID"])
