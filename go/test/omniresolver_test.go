...
import (
	...
	omni "github.com/voxgig-sdk/jojos-bizarre-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.JojosBizarreError); is {
				out := map[string]any{
					"name":    "JojosBizarreError",
...
func omniCtx(arg any, client *sdk.JojosBizarreSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.JojosBizarreSDK); is {
			client = live
