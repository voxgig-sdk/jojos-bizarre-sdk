# JojosBizarre Golang SDK



The Golang SDK for the JojosBizarre API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/jojos-bizarre-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/jojos-bizarre-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/jojos-bizarre-sdk/go=../jojos-bizarre-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *JojosBizarreSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### JojosBizarreSDK methods
...
| `Character` | `(data map[string]any) JojosBizarreEntity` | Create a Character entity instance. |
| `Stand` | `(data map[string]any) JojosBizarreEntity` | Create a Stand entity instance. |

### Entity interface (JojosBizarreEntity)

All entities implement the `JojosBizarreEntity` interface.
...
Create an instance: `stand := client.Stand(nil)`
...
```go
stand, err := client.Stand(nil).Load(map[string]any{"id": "stand_id"}, nil)
if err != nil {
    panic(err)
}
fmt.Println(stand) // the loaded record
```

#### Example: List

```go
stands, err := client.Stand(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(stands) // the array of records
```
...
```
github.com/voxgig-sdk/jojos-bizarre-sdk/go/
├── jojos-bizarre.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/jojos-bizarre-sdk/go`) re-exports everything needed
