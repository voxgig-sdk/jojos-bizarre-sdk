...
class JojosBizarreTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            JojosBizarreTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = JojosBizarreTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = JojosBizarreTestRunner.getenv("JOJOS_BIZARRE_TEST_LIVE")
        override = JojosBizarreTestRunner.getenv("JOJOS_BIZARRE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = JojosBizarreTestRunner.getenv(key)
        ...
        explain = JojosBizarreTestRunner.getenv("JOJOS_BIZARRE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if JojosBizarreTestRunner._test_control is not None:
            return JojosBizarreTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                JojosBizarreTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            JojosBizarreTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = JojosBizarreTestRunner.load_test_control()
...
def load_env_local():
    JojosBizarreTestRunner.load_env_local()


def env_override(m):
    return JojosBizarreTestRunner.env_override(m)


def entity_data(v):
    return JojosBizarreTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return JojosBizarreTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return JojosBizarreTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return JojosBizarreTestRunner.load_test_control()


def live_client_options():
    return JojosBizarreTestRunner.live_client_options()


def live_delay_ms():
    return JojosBizarreTestRunner.live_delay_ms()
