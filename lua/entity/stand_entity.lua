...
local StandEntity = {}
StandEntity.__index = StandEntity


function StandEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, StandEntity)
  self._name = "stand"
  self._client = client
...
function StandEntity:get_name()
  return self._name
end


function StandEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return StandEntity.new(self._client, opts)
end
...
function StandEntity:mark_deleted()
  self._deleted = true
end


function StandEntity:deleted()
  return true == self._deleted
end


function StandEntity:data_set(args)
  if args ~= nil then
...
function StandEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function StandEntity:match_set(args)
  if args ~= nil then
...
function StandEntity:match_get()
...
function StandEntity:stream(action, args, callopts)
...
---@param reqmatch StandLoadMatch
---@param ctrl? table
---@return Stand
---@return string? err
function StandEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqmatch StandListMatch
---@param ctrl? table
---@return Stand[]
---@return string? err
function StandEntity:list(reqmatch, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "list",
    ctrl = ctrl,
...
function StandEntity:_run_op(ctx, post_done)
  local utility = self._utility
