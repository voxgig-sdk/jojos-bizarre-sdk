...
local JojosBizarreSDK = {}
JojosBizarreSDK.__index = JojosBizarreSDK
...
JojosBizarreSDK._make_feature = _make_feature


function JojosBizarreSDK.new(options)
  local self = setmetatable({}, JojosBizarreSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function JojosBizarreSDK:options_map()
...
function JojosBizarreSDK:get_utility()
  return Utility.copy(self._utility)
end


function JojosBizarreSDK:get_root_ctx()
  return self._rootctx
end


function JojosBizarreSDK:prepare(fetchargs)
...
function JojosBizarreSDK:direct(fetchargs)
...
-- by passing it.
function JojosBizarreSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function JojosBizarreSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "JojosBizarreSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function JojosBizarreSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = JojosBizarreSDK.new(sdkopts)
  sdk.mode = "test"
