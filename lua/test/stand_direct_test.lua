...
describe("StandDirect", function()
  it("should direct-list-stand", function()
    local setup = stand_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-stand", setup.live and "live" or "unit")
  ...
  it("should direct-load-stand", function()
    local setup = stand_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-stand", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function stand_direct_setup(mockres)
