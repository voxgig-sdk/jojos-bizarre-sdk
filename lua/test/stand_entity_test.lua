...
describe("StandEntity", function()
  it("should create instance", function()
    local testsdk = sdk.test(nil, nil)
    local ent = testsdk:Stand(nil)
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Stand(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Stand(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    local setup = stand_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"list", "load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "stand." .. _op, _live and "live" or "unit")
    ...
    local stand_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.stand")))
    local stand_ref01_data = nil
    if #stand_ref01_data_raw > 0 then
      stand_ref01_data = helpers.to_map(stand_ref01_data_raw[1][2])
    end

    -- LIST
    local stand_ref01_ent = client:Stand(nil)
    local stand_ref01_match = {}

    local stand_ref01_list_result, err = stand_ref01_ent:list(stand_ref01_match, nil)
    assert.is_nil(err)
    assert.is_table(stand_ref01_list_result)

    -- LOAD
    local stand_ref01_match_dt0 = {
...
function stand_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/stand/StandTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read stand test data: " .. entity_data_file)
  end
